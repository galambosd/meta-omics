...
parser = argparse.ArgumentParser(description = "This script parses nearest archaeal 'Bin_*-mean_coverage.txt' files to output " + 
"the normalized mean DNA read coverage of each bin at each sample site. Outputs 'bin_by_bin_normalized_cov.txt.'\n\nAdditional required files:/n/t'sample_names_archaea.txt -- list sample numbers and names to be included"+
"\n\t'bin_taxonomy.txt -- list the genus of each bin'\n\t'normalization_factors_reads_DNA.txt -- list of sample numbers and normalization factors\n\t'bin_list.txt' -- list bins for each sample site'")
...
norm_file = open('normalization_factors_reads_DNA.txt', 'r')
...
outfile.write("DNA coverages across samples in '{0}' normalized with '{1}.'\n".format(samples_file_name, norm_file.name))
...
for FS in FS_list:
    ...
    for bin in bin_list:
        ...
        for sample, val in zip(samples, vals):
        	# get normalized coverage by dividing the mean coverage from the file by the # of DNA reads in that mapping
        	sample_name = sample[0:5]
        	#doing DNA -- skip the ones that are RNA
        	if 'RNA' in sample:
        		continue
        	# skip the samples that don't have corresponding RNA reads
